...
public class GameViewModel {
	...
	public int getPathCount() {
		...
		if (adventureMove <= monsterMove) {
			...
			rowsToMove = Math
					.abs(Repository.getInstance().getAdventure().getRow() - Repository.getInstance().getTriger().getRow());
			columnsToMove = Math.abs(
					Repository.getInstance().getAdventure().getColumn() - Repository.getInstance().getTriger().getColumn());
			trigerMove = rowsToMove + columnsToMove;
			if (trigerMove <= monsterMove) {
				rowsToMove = Math
						.abs(Repository.getInstance().getTriger().getRow() - Repository.getInstance().getGold().getRow());
				columnsToMove = Math.abs(
						Repository.getInstance().getTriger().getColumn() - Repository.getInstance().getGold().getColumn());
				return trigerMove+rowsToMove+columnsToMove;
			}else {
				return -1;
			}
		}
